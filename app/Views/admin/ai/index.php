<div class="space-y-6">
    ...
    <!-- Image Stats (synced with admin/images) -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg shadow p-4">
            <p class="text-sm text-neutral-500">Draft (Need AI)</p>
            <p class="text-2xl font-semibold text-yellow-600"><?= $imageStats['draft'] ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <p class="text-sm text-neutral-500">In Queue</p>
            <p class="text-2xl font-semibold text-blue-600"><?= $queueStats['pending'] + $queueStats['processing'] ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <p class="text-sm text-neutral-500">Published</p>
            <p class="text-2xl font-semibold text-green-600"><?= $imageStats['published'] ?></p>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <p class="text-sm text-neutral-500">Failed</p>
            <p class="text-2xl font-semibold text-red-600"><?= $queueStats['failed'] ?></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <form action="<?= $view->url('/admin/ai/process') ?>" method="POST" class="inline">
                ...
                <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700"
                        <?= $queueStats['pending'] == 0 ? 'disabled' : '' ?>>
                    Process 5
                </button>
            </form>
            <form action="<?= $view->url('/admin/ai/process-all') ?>" method="POST" class="inline">
                <?= $view->csrf() ?>
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                        <?= $queueStats['pending'] == 0 ? 'disabled' : '' ?>>
                    Process All (<?= $queueStats['pending'] ?>)
                </button>
            </form>
        ...
        <div class="text-center py-6">
            ...
            <a href="<?= $view->url('/admin/settings?group=api') ?>" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-neutral-200">
            <h2 class="text-lg font-semibold">Processing Queue (<?= count($queue) ?>)</h2>
        </div>
        <div class="p-6">
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-4 p-3 bg-neutral-50 rounded-lg">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-neutral-500">
                            ...
                            <?php if ($item['error_message']): ?>
                                <span class="text-red-500">| <?= e(substr($item['error_message'], 0, 100)) ?></span>
                            <?php endif; ?>
                        </p>
                    </div>
                    ...
                    <form action="<?= $view->url('/admin/ai/process/' . $item['image_id']) ?>" method="POST" class="flex-shrink-0">
                        <?= $view->csrf() ?>
                        <button type="submit" class="px-3 py-1 text-xs bg-primary-600 text-white rounded hover:bg-primary-700">
                            Process
                        </button>
                    </form>
                    <?php endif; ?>
    ...
    <!-- Draft Images Not in Queue -->
    <?php if (!empty($unqueued)): ?>
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-neutral-200 flex justify-between items-center">
            <h2 class="text-lg font-semibold">Draft Images Not Queued (<?= count($unqueued) ?>)</h2>
            <form action="<?= $view->url('/admin/ai/queue') ?>" method="POST">
                <?= $view->csrf() ?>
                <button type="submit" class="px-3 py-1.5 text-sm bg-primary-600 text-white rounded hover:bg-primary-700">
                    Queue All
                </button>
            </form>
        </div>
    ...
    <!-- Empty State -->
    <?php if (empty($queue) && empty($unqueued) && $queueStats['failed'] == 0): ?>
    <div class="bg-white rounded-lg shadow p-12 text-center">
        <svg class="w-16 h-16 mx-auto text-green-500 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
        </svg>
        <h3 class="text-lg font-semibold text-neutral-800 mb-2">All Caught Up!</h3>
        <p class="text-neutral-600">No images waiting for AI processing. Upload new images to get started.</p>
        <a href="<?= $view->url('/admin/images/upload') ?>" class="inline-block mt-4 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">
            Upload Images
        </a>
    </div>
    <?php endif; ?>
